...
class SnapOpenPluginInstance:
	...
	def _insert_menu( self ):
		...
		snapopen_action = gtk.Action( name="SnapOpenAction", label="Go to File by name...\t", tooltip="Go to a file with regex search", stock_id=gtk.STOCK_JUMP_TO )
  ...
  # UI DIALOGUES
	...
	def on_snapopen_action( self ):
		...
		if fbroot != "" and fbroot is not None:
			...
			self._snapopen_window.set_title("Find a file by name (Filebrowser integration)")
		else:
			eddtroot = self.get_eddt_root()
			if eddtroot != "" and eddtroot is not None:
				self._rootdir = eddtroot
				self._snapopen_window.set_title("Find a file by name (EDDT integration)")
			else:
				self._snapopen_window.set_title("Find a file by name (cwd): " + self._rootdir)		
